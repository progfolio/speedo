...
(defvar speedo--mock-data '(:segments (( :name "BulletPromenade")
                                       ( :name "Super Koopa Hills")
                                       ( :name "Cloon Cavern")
                                       ( :name "The Toxic Passage (secret)")
                                       ( :name "Tubba's Palace")
                                       ( :name "The Toxic Passage")
                                       ( :name "Shiverthorn Hollow")
                                       ( :name "Delfino Shores (secret)")
                                       ( :name "Pickle's Palace")
                                       ( :name "Delfino Shores")
                                       ( :name "Labrys Abyss")
                                       ( :name "The Watcher's Keep")
                                       ( :name "Gridiron Ridge")
                                       ( :name "Bridge of Moths")
                                       ( :name "Forest of Abkhazia")
                                       ( :name "The Living Earth (secret)")
                                       ( :name "Grim Shade Manor")
                                       ( :name "Ossuary Rift")
                                       ( :name "Whynot's Lookout")
                                       ( :name "Underscore Burrows")
                                       ( :name "Forgotten Acropolis")
                                       ( :name "The Gateway")
                                       ( :name "Twilight Bridge")
                                       ( :name "Celestial Rex")
                                       ( :name "Australian Airways")
                                       ( :name "Stormcrow's Alcazar")
                                       )
                                      :attempts ((:start 1616642685487 :splits
                                                         ((:segment "BulletPromenade" :duration 60800)
                                                          (:segment "Super Koopa Hills" :duration 46670)
                                                          (:segment "Cloon Cavern" :duration 84750)
                                                          (:segment "The Toxic Passage (secret)" :duration 66030)
                                                          (:segment "Tubba's Palace" :duration 64480)
                                                          (:segment "The Toxic Passage" :duration 47120)
                                                          (:segment "Shiverthorn Hollow" :duration 71570)
                                                          (:segment "Delfino Shores (secret)" :duration 53390)
                                                          (:segment "Pickle's Palace" :duration 56710)
                                                          (:segment "Delfino Shores" :duration 74030)
                                                          (:segment "Labrys Abyss" :duration 93700)
                                                          (:segment "The Watcher's Keep" :duration 78780)
                                                          (:segment "Gridiron Ridge" :duration 89100)
                                                          (:segment "Bridge of Moths" :duration 60050)
                                                          (:segment "Forest of Abkhazia" :duration 78360)
                                                          (:segment "The Living Earth (secret)" :duration 56900)
                                                          (:segment "Grim Shade Manor" :duration 68110)
                                                          (:segment "Ossuary Rift" :duration 70450)
                                                          (:segment "Whynot's Lookout" :duration 11452)
                                                          (:segment "Underscore Burrows" :duration 12225)
                                                          (:segment "Forgotten Acropolis" :duration 100880)
                                                          (:segment "The Gateway" :duration 84740)
                                                          (:segment "Twilight Bridge" :duration 74890)
                                                          (:segment "Celestial Rex" :duration 125870)
                                                          (:segment "Australian Airways" :duration 113760)
                                                          (:segment "Stormcrow's Alcazar" :duration 61410)))))
  "Mock database.")
          ...
          (when-let* ((pb (speedo--run-pb))
                      (pb-duration
                       (speedo--splits-duration
                        (seq-take (plist-get pb :splits)
                                  (1+ speedo--segment-index))))
                      (current (if in-progress
                                   speedo--attempt-current
                                 (car (last (plist-get speedo--data :attempts)))))
                      (current-duration (if in-progress
                                            (- (speedo--timestamp) (plist-get current :start))
            ...
            (when (or losing behind)
              (when-let ((current-relative (text-property-search-forward 'current-relative-timer)))
                (put-text-property (prop-match-beginning current-relative)
                                   ...
                                   'display (speedo--relative-time pb-duration current-duration)))))
